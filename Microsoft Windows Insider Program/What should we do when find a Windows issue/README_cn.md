...
1. 在**不同的 Windows 版本系统**下进行测试，确认问题能否成功复现 (**例如**：如果此问题是在 Windows 预览体验版本中发现的，那么就测试一下在正式版本 Windows 中此问题是否仍然存在)。

2. 执行**类似**的操作，确认问题能否成功复现 (**例如**：如果此问题是在浏览器中输入文本时发现的，那么就测试一下在本地的 txt 文本文档中输入文本时问题是否仍然存在)。
